 ...
 * Kept the original path data but wrapped in a cleaner SVG structure
 * and added drop-shadows via CSS classes for depth.
 */
export const NotFoundSvg = ({ className, ...props }: React.ComponentProps<"svg">) => {
  return (
    <div className={cn("relative z-10", className)}>
      {/* Background Glow Effect */}
      <div className="absolute inset-0 bg-blue-500/20 blur-3xl rounded-full opacity-50 -z-10 transform scale-75" />
...
export default function NotFoundContainer({
  ...
  return (
    <div className="relative min-h-[80vh] w-full flex items-center justify-center overflow-hidden">
      ...
      <main 
        className={cn(
          "relative z-10 w-full max-w-(--max-app-width) px-6 py-12 mx-auto",
          className
        )}
      >
        <section className="flex flex-col-reverse lg:flex-row items-center justify-between gap-12 lg:gap-24">
          ...
          <div className="flex-1 space-y-8 text-center lg:text-left max-w-xl">
            <div className="space-y-4">
              ...
              <p className="text-lg lg:text-xl text-muted-foreground text-balance leading-relaxed">
                {description || "Sorry, we couldn’t find the page you’re looking for. It might have been moved, deleted, or you may have mistyped the address."}
              </p>
            </div>

            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-2">
              <PreviousPageLink 
                 className="h-11 px-8 rounded-full border border-input bg-background hover:bg-accent hover:text-accent-foreground transition-colors font-medium flex items-center gap-2"
              />
              ...
              <ButtonLink
                ...
                variant="default"
                href="/"
                className="h-11 px-8 shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all duration-300"
                {...props}
              >
                {children ? (
                  children
          ...
          <div className="flex-1 w-full max-w-lg flex justify-center lg:justify-end">
             <NotFoundSvg className="w-full h-auto max-h-[400px] pointer-events-none select-none transition-transform hover:scale-105 duration-700 ease-in-out" />
          </div>
